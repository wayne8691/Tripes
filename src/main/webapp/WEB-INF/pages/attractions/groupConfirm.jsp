...
<script type="text/javascript">	
	...
	window.onload = function() {
		...
		xhr1.open("GET", "<c:url value='/travelGroup/" + id + "' />", true);
		xhr1.send();	
		xhr1.onreadystatechange = function() {
		 // 伺服器請求完成
		    if (xhr1.readyState == 4 && xhr1.status == 200) {
			    groupData = JSON.parse(xhr1.responseText);
			   console.log(groupData)
			   div_image.style ="background-image:url("+ groupData.image +");";
			   name.innerHTML = groupData.name;
			   price.innerHTML = groupData.price;
			   origin.innerHTML = groupData.origin;
			   destination.innerHTML = groupData.destination;
			   day.innerHTML = groupData.day;
			   feature.innerHTML = groupData.feature;
			   //日期
			   var departure =document.getElementById("setOffDay");
			   content="<option>請選擇日期</option>";
			   for(var i=0; i < groupData.travelGroupBeanId.length; i++){
					 content +="<option value='"+ i  +"'>"+  groupData.travelGroupBeanId[i].setoffday +"</option>";
				}
			   departure.innerHTML = content;
		    }
	    }
...
//	 	xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
	...
	//顯示可報名人數	
	function seachdate(){
		var departurevalue =document.getElementById("setOffDay").value;
		var sum =document.getElementById("sum");
		 
		console.log(departurevalue);
		console.log(sum);
			//剩餘可報名人數
			remaining_value =groupData.travelGroupBeanId[departurevalue].remaining ;
			//總人數
			sum_value =groupData.travelGroupBeanId[departurevalue].sum ;
		
			sum.innerHTML = " 可報名數: "+remaining_value + " / " + sum_value +" 人";
		}
...
// 	     var departure =document.getElementById("departure");
	     var departurevalue =document.getElementById("setOffDay").value;
		 var xhr = new XMLHttpRequest();
			xhr.open("POST", "<c:url value='/tricket.controller' />", true);
			xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
			xhr.send('id='+pk+"&date="+groupData.travelGroupBeanId[departurevalue].setoffday+"&did="+groupData.travelGroupBeanId[departurevalue].id);
			xhr.onreadystatechange = function(){
			 if(xhr.readyState==4&&xhr.status==200){
					...
					if(products.member=="notmember"){
						...
						remaining_value--;
						var sum =document.getElementById("sum");
						sum.innerHTML = " 可購買數: "+remaining_value + " / " + sum_value +" 人";
					 swal({
				...
				<div class="col-md-6 pl-md-5 py-5" >
				   <div class="row justify-content-start pb-3">
				       <div class="col-md-12 heading-section ftco-animate fadeInUp ftco-animated">
				         <h2 class="mb-4">
				         	<font style="vertical-align: inherit;" id="name"></font>
				         </h2>
				         <h4 class="mb-4">出發地點 : 
					     ...
					     <h3 class="mb-4" style="color:red;" >優惠價 : $
					     	<span id="price"></span>
					     	<span>/每位</span>
					     </h3>  
					     <div class="mb-4">
						 	<div class="form-field">
								<span style="font-size:20px;"> 選擇旅遊日期 : </span>
								<select id="setOffDay" name="setTodaysDate" onchange='seachdate()'>								   
								</select>
								<span id='sum' style="font-size:20px;"></span>
							</div>	
						 </div>	
					
					     <p> <a class="btn btn-primary" onclick='shoppingcart();'>
							      <font style="vertical-align: inherit;" > 加入購物車</font>
					         </a>
...
<!--    //////////////googleMap/////////////////////	     -->
<section class="ftco-section services-section" style='background-color:#FCFCFC;'>
	<div class="container">
