...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$.ajax({
			type:'post',
			url:'/attractions/findAllAttractionsByCountDesc/',
			dataType:'JSON',
					...
					if(json = null){
					    ...
					    $.each(parsedObjinArray,function(i,n){
					    	 data1 = "<br><div><div class='item'><div class='project-destination'>"
										    + "<a href='" + userAtDisplay + n.pk + "' class='img' style='background-image: url(" +  n.image  + ");'>"
	...
	var lis=null;
						    ...
						    $.each(parsedObjinArray,function(i,n){
						    	 data1 = "<div class='col-md-4 ftco-animate fadeInUp ftco-animated'>"
									    	 +"<div class='project-wrap'>"
												+ "<a href='" + userAtDisplay + n.pk + "' class='img' style='background-image: url(" +  n.image  + ");'>"
													+ "<span class='price'>" +  n.counties + "</span>"
												+ "</a>"
												+ "<div class='text p-4'>"
											    	+ "<h3 hidden='hidden'>"+ n.pk +"</h3>"
											    	+ "<h3>"+ n.name +"</h3>"
											    	+ "<h4 class='ellipsis'> "+ n.introduction + "</h4>"
											    	+ "<p><span class='fa fa-map-marker'> " + n.address + "</span></p>"
									    	+ "</div> </div> </div>";
								  area1.append(data1);
						   });
					  }
	    ...
	    function search(){
			...
			$.ajax({
				type:'post',
				url:'/attractions/findAttractionsByNameOrCounties/'+ searchName +'/'+ searchCounties,
				dataType:'JSON',
				contentType:'application/json',
				success: function(data) {
					var json = JSON.stringify(data, null, 4);
				    ...
				    $('#area1').empty(""); //清空table內的資料
						if(json = null){
					    	$('#area1').prepend("<span>搜尋到0筆資料</span>");
					    }else{
							userAtDisplay = "<c:url value='/userAtDisplay/' />";
						    $.each(parsedObjinArray,function(i,k){
						    	 data2 = "<div class='col-md-4 ftco-animate fadeInUp ftco-animated'>"
									    	 +"<div class='project-wrap'>"
												+ "<a href='" + userAtDisplay + k.pk + "' class='img' style='background-image: url(" +  k.image  + ");'>"
													+ "<span class='price'>" +  k.counties + "</span>"
												+ "</a>"
												+ "<div class='text p-4'>"
											    	+ "<h3 hidden='hidden'>"+ k.pk +"</h3>"
											    	+ "<h3>"+ k.name +"</h3>"
											    	+ "<h4 class='ellipsis'> "+ k.introduction + "</h4>"
											    	+ "<p><span class='fa fa-map-marker'> " + k.address + "</span></p>"
									    	+ "</div> </div> </div>";
								$('#area1').append(data2);
						   });
					  }
			...
			<div class="row no-gutters slider-text js-fullheight align-items-end justify-content-center" style="height: 937px;">
				<div class="col-md-9 ftco-animate pb-5 text-center fadeInUp ftco-animated">
					...
					<p class="breadcrumbs">
						...
						<span class="mr-2"><a href="<c:url value='/showAllAttractions.UserAtHome'/>">旅遊景點<i class="fa fa-chevron-right"></i></a></span> 
					</p>
					<h1 class="mb-0 bread">旅遊景點</h1>
				</div>
			</div>
		</div>
	</section>
...
<section class="ftco-section-1  ftco-no-pb" style='background-color:#F0F0F0;height:105em;'>
   ...
   <!--  搜尋列  --> 
      <div class="row">
       <div class="col-md-12" id="fontsize">
          ...
          <div class="row no-gutters">
			...
			<div class="col-lg d-flex">
				<div class="form-group d-flex w-100 border-1">
				    <div class="form-field w-100 align-items-center d-flex">
				        <input type ="button" value="顯示所有景點"  onclick="javascript:location.href='<c:url value='/showAllAttractions.UserAtHome'/>#gotoTitle'" class="align-self-stretch form-control btn btn-primary" style="font-size:20px;"></input>
				    </div>
				</div>
			</div>
			<div class="col-lg d-flex">
				<div class="form-group d-flex w-100 border-1">
				    <div class="form-field w-100 align-items-center d-flex">
				        <input type ="button" value="顯示所有餐廳" onclick="javascript:location.href='<c:url value='/showAllRestaurant.UserReHome'/>#gotoTitle'"  class="align-self-stretch form-control btn btn-primary" style="font-size:20px;"></input>
				    </div>
				</div>
			</div>
			</div>
	  ...
	  <div class="col text-center">
		    <div class="block-27" id="showpage">
			      ...
			      <c:forEach var="i" begin="1" end="${totalPages}" step="1">
			        <li class="li" id="${i}" name='pages' value="${i}" onclick="change(${i},this)"><a href="#gotoTitle"><span>${i}</span></a></li>  
			      </c:forEach>
