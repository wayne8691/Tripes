...
<div class="col-md-4 col-lg-4">
	<div class="project-wrap hotel">
		<a href="#" class="img" style="background-image: url(<%=room.getPic()%>);">
			<span class="price" style="font-size:20px;">NTD $<%=String.valueOf(room.getPrice().intValue())%>/晚</span>
		</a>
		<div class="text p-4">
			...
			<button class="btn btn-primary"  value='<%=room.getId()%>' onclick='buy(this)'>訂房</button>		
		</div>
	</div>
</div>
<%
...
<div class="row mt-5">
	  <div class="col text-center">
	    <div class="block-27"">
			<ul>
				<li><a href="javascript:changePage(<%=(pageNow - 1) < 1 ? 1 : (pageNow - 1)%>)">&lt;</a></li>
				<%
				for (int i = 0; i < maxPage; i++) {
				%>
				<li><a href="javascript:changePage(<%=(i+1)%>)"><%=(i+1)%></a></li>
				<%
				}
				%>
				<li><a href="javascript:changePage(<%=(pageNow + 1) > maxPage ? maxPage : (pageNow + 1)%>)">&gt;</a></li>
			</ul>
		</div>
	</div>
</div>